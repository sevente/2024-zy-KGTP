...
if __name__ == "__main__":
    ...
    save_path = f"/home/sdb/xx/path/LLM/ChatgptForD2T/{dataset}/"
    pred_file = os.path.join(save_path, "test")
    pred_text = open(pred_file, "r").readlines()
    print("predicted.txt :", len(pred_text))

    results = metric_with_bleu(gt_text_list, pred_text, dataset=dataset)
    results_rouge = metric_with_rouge(gt_text_list, pred_text, dataset=dataset)
