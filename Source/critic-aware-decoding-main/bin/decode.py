...
if __name__ == "__main__":
    ...
    if args.model_name:
        ...
        if args.wrapper:
            if args.wrapper == "data":
                print("experiment -> wrapper -> data")
                di = CriticGenDataInferenceModule(args, None, None, wrapper=args.wrapper)
            elif args.wrapper == "data-full":
                print("experiment -> wrapper -> data-full")
                args.wrapper = "data"
                di = CriticAwareInferenceModule(args, None, None, wrapper=args.wrapper)
            else:
                print("experiment -> wrapper -> else")
                ...
                critic_model = ClassificationModule.load_from_checkpoint(checkpoint_path=args.critic_ckpt,
                                                                         model=SimpleClassifierModelWithBNSELU(
                                                                             'xlm-roberta-base'))
                critic_model.eval()
                if args.fintuned_model:
                    di = CriticAwareInferenceModule(args, critic_model, critic_tokenizer, model_path=None,
                                                    wrapper=args.wrapper)
                else:
                    di = CriticAwareInferenceModule(args, critic_model, critic_tokenizer, model_path=model_path,
                                                    wrapper=args.wrapper)
        else:
            print("experiment -> no wrapper")
    ...
    if not os.path.exists(os.path.join(args.exp_dir, args.experiment)):
        os.makedirs(os.path.join(args.exp_dir, args.experiment))
