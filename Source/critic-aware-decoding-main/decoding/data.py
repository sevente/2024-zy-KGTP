...
class WebNLG(Dataset):
    ...
    def load(self, splits, path):
        ...
        dataset = load_dataset("gem", "web_nlg_en", cache_dir="/home/sdb/xx/path/datasets/HFdatasets/cache")

        for split in splits:
            data = dataset[split if split != "dev" else "validation"]
            print(data)
            print("dataset input: ", data[0]["input"], "len: ", len(data[0]["input"]))
            if split == "test":
                print("dataset references of test dataset: ", data[0]["references"], "len: ", len(data[0]["references"]))
            else:
                print("dataset target of train or validation: ", data[0]["target"], "len: ", len(data[0]["target"]))

            for example in data:
                triples = example["input"]
                print(triples, "len of triples: ", len(triples))
                triples = [t.split("|") for t in triples] # 一个sample可能有多种triple关系
                ...
                entry = DataEntry(data=triples, refs=refs, data_type="triples") #将triples和refs封装成一个class
