...
class InferenceModule(pl.LightningModule):
    def __init__(self, args, training_module_cls, model_path=None):
        ...
        if model_path is not None:
            print("model_path in InferenceModule: ", model_path)
            # 先执行 load_from_checkpoint， 然后执行training_module_cls 的 __init__，
            self.model = training_module_cls.load_from_checkpoint(model_path)
            self.model.freeze()
            self.model_name = self.model.model.name_or_path
            logger.info(f"Loaded model from {model_path}")
            print("model_name if model_path is not None: ", self.model_name)
        elif args.fintuned_model is not None and args.model_name is None:
            self.model = training_module_cls(args)
            self.model_name = self.model.model.name_or_path
            print("self.model_name if args.fintuned_model: ", self.model_name)
        else:
            print("No model_path")
