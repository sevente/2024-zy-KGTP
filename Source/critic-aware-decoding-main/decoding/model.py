...
class TrainingModule(pl.LightningModule):
    def __init__(self, args, **kwargs):
        ...
        if args.fintuned_model is not None:
            self.tokenizer = AutoTokenizer.from_pretrained(args.fintuned_model, use_fast=True)
        else:
            self.tokenizer = AutoTokenizer.from_pretrained(args.model_name, use_fast=True)
        # self.tokenizer = AutoTokenizer.from_pretrained("/home/sdb/xx/path/modelZooHuggingFace/bart-base", use_fast=True)
    ...
    def __init__(self, args, **kwargs):
        ...
        if args.fintuned_model is not None:
            if hasattr(args, "load_in_8bit") and args.load_in_8bit:
                print("args.model_name in Seq2SeqTrainingModule w args.fintuned_model", args.model_name)
                self.model = AutoModelForSeq2SeqLM.from_pretrained(
                    args.fintuned_model, device_map="auto",
                    load_in_8bit=True,
                    return_dict=True,
                    use_safetensors=True)
            else:
                print("args.model_name in Seq2SeqTrainingModule w args.fintuned_model", args.model_name)
                self.model = AutoModelForSeq2SeqLM.from_pretrained(args.fintuned_model,
                                                                   return_dict=True,
                                                                   use_safetensors=True)
        else:
            if hasattr(args, "load_in_8bit") and args.load_in_8bit:
                print("args.model_name in Seq2SeqTrainingModule", args.model_name)
                # self.model = AutoModelForSeq2SeqLM.from_pretrained(args.model_name, device_map="auto", load_in_8bit=True,
                #                                                    return_dict=True)
                self.model = AutoModelForSeq2SeqLM.from_pretrained(
                    "/home/sdb/xx/path/modelZooHuggingFace/bart-base", device_map="auto",
                    load_in_8bit=True,
                    return_dict=True)
            else:
                print("args.model_name in Seq2SeqTrainingModule", args.model_name)
                # self.model = AutoModelForSeq2SeqLM.from_pretrained(args.model_name, return_dict=True)
                self.model = AutoModelForSeq2SeqLM.from_pretrained("/home/sdb/xx/path/modelZooHuggingFace/bart-base",
                                                                   return_dict=True)
