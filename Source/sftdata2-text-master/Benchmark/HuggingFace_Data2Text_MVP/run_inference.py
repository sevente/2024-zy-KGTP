...
def get_args():
    ...
    args.add_argument('-b', '--batch_size', default=200,
                      type=int)
    args.add_argument('-d', '--dataset_name', default='webnlg17',
                      type=str)
...
if __name__ == "__main__":
    ...
    elif args.model_name == 'mvp-data-to-text':
        model = MvpForConditionalGeneration.from_pretrained(args.model_path)
        tokenizer = AutoTokenizer.from_pretrained(args.model_path)
    model.to(device)

    # gt_file = os.path.join(args.test_file, "test_y_prompt.txt")
    # test_in_file = os.path.join(args.test_file, "test_x_prompt.txt")
    # gt_file = os.path.join(args.test_file, "val_15_y_prompt.txt")
    # test_in_file = os.path.join(args.test_file, "val_15_x_prompt.txt")


    if args.dataset_name in ["webnlg17", "webnlg20"]:
        # test_in_file = os.path.join(args.test_file, "test_both_x_prompt.txt")
        # gt_file = os.path.join(args.test_file, "test_both_y_prompt.txt")
        test_in_file = os.path.join(args.test_file, "test_seen_x_prompt.txt")
        gt_file = os.path.join(args.test_file, "test_seen_y_prompt.txt")
        # test_in_file = os.path.join(args.test_file, "test_unseen_x_prompt.txt")
        # gt_file = os.path.join(args.test_file, "test_unseen_y_prompt.txt")
    elif args.dataset_name in ["e2e_clean", ]:
        test_in_file = os.path.join(args.test_file, "test_x_prompt.txt")
        gt_file = os.path.join(args.test_file, "test_y_prompt.txt")
    elif args.dataset_name in ["DART", ]:
        test_in_file = os.path.join(args.test_file, "test_both_x_prompt.txt")
        gt_file = os.path.join(args.test_file, "test_both_y_prompt.txt")
    elif args.dataset_name in ["webnlg", "webnlg2", "e2e", "dart"]:
        test_in_file = os.path.join(args.test_file, "test_x_prompt.txt")
        gt_file = os.path.join(args.test_file, "test_y_prompt.txt")
    ...
    results = metric_with_bleu([gt_text, ], all_pred, args.dataset_name)
    print(results)
